...
def main():
    ...
    indices = torch.arange(1024) #number of training examples

    # creating subset for test purposes for shorter runtime.
    trainSubset = data_utils.Subset(dataset1, indices)
    
    dataset2 = datasets.MNIST('../data', train=False,
                       transform=transform)
    train_loader = torch.utils.data.DataLoader(trainSubset, **train_kwargs)
    ...
    with torchprof.Profile(model, use_cuda=True, profile_memory=True) as prof:
        for epoch in range(1, args.epochs + 1):
            ...
            if device.type == 'cuda':
                ...
                print('Cached:   ', torch.cuda.memory_cached()/1024**3, 'GB')
